...
class AlphaVantageAPI:
    ...
    def get_daily(self, ticker, output_size = 'full'):
        ...
        if 'Time Series (Daily)' not in response_data.keys():
            raise Exception(
                f"Invalid API call, Check the ticker symbol '{ticker}' is correct or not."
            )
...
class SQLRepository:
    def __init__(self, connection = sqlite3.connect(database = settings.db_name, check_same_thread = False)):
