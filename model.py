...
class GarchModel:
    ...
    def fit(self, p, q):
        ...
        self.model = arch_model(self.data , p=p, q=q, rescale=False).fit(disp=False)
        

    def __clean_prediction(self, prediction):
        """Reformat model prediction to JSON.(Helper Function)
    ...
    def dump(self):
        ...
        timestamp = pd.Timestamp.now().strftime("%Y-%m-%dT%H-%M-%S.%f")[:-3]
